    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data.items) && data.items.length > 0) {
          console.log(`[Cloudflare Edge Worker] Fetched ${data.items.length} items for @${username}`);
      ...
      // fall through to Hybrid Scraper
    }
  }

  // Strategy 2: Hybrid Profile Scraper (Direct Instagram Profile HTML Shortcodes + RSS Bridge)
  try {
    const nodeMap = new Map<string, any>();

    // 1. Direct Instagram Profile HTML: Extract preloaded shortcodes and CDN images
    try {
