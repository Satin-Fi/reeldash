...
async function capture() {
  ...
  // Navigate to first reel page
  await page.goto('https://reeldash-nine.vercel.app/reels', { waitUntil: 'load' });
  await new Promise(r => setTimeout(r, 2000));

  const reelLink = await page.evaluate(() => {
    const a = document.querySelector('a[href^="/reel/"]');
    return a ? a.getAttribute('href') : null;
  });

  if (reelLink) {
    await page.goto(`https://reeldash-nine.vercel.app${reelLink}`, { waitUntil: 'load' });
    await new Promise(r => setTimeout(r, 2000));
    await page.screenshot({ path: 'C:/Users/Piyush/.gemini/antigravity/brain/8440ca0a-61da-4c07-a877-85f850d39e18/reel_page_clean.png' });
    console.log('Reel page screenshot captured!');
  }
